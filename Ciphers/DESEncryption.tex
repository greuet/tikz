...
\begin{scope}[node distance = 1.2cm]
  ...
  \node[box, below = of IP.west] (L0) {$L_0$};
  \node[box, below = of IP.east] (R0) {$R_0$};
...
\end{scope}
\draw[->, thick] (input) -- (IP);
\draw[->, thick] (IP) |- ($(IP)!0.5!(L0)$) -| (L0.north);
\draw[->, thick] (IP) |- ($(IP)!0.5!(R0)$) -| (R0.north);
...
\node[box rounded, below of = middle16] (IPinv) {Inverse Initial Permut};
\begin{scope}[node distance = 1.2cm]
\node[box large, below of = IPinv] (output) {Output};
\end{scope}

\draw[<-, thick] (IPinv) |- ($(IPinv)!0.5!(L16)$) -| (L16.south);
\draw[<-,thick] (IPinv) |- ($(IPinv)!0.5!(R16)$) -| (R16.south);
\draw[->, thick] (IPinv) -- (output);
